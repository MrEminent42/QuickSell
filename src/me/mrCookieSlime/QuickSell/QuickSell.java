...
public class QuickSell extends JavaPlugin {
	...
	public void onEnable() {
		...
		if (loader.load()) {
			...
			local.setDefault("editor.set-permission-shop", "&a&l! &7Please type in a Permission for your Shop!",
			...
			local.setDefault("booster.extended.EXP",
					"&6%player% &ehas extended the %multiplier%x Booster (Experience) for %time% more Minute/s");
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (cmd.getName().equalsIgnoreCase("sell")) {
			if (cfg.getBoolean("options.enable-commands")) {
				...
				local.sendTranslation((Player) sender, "commands.disabled", false);
				...
				if (args.length >= 1) {
					if (args[0].equalsIgnoreCase("reload")) {
						...
						if (npc == null)
							...
							sender.sendMessage(npc.getName() + " §cis not linked to any Shop!");
